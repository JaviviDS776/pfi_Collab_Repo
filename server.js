...
app.use(session({
  ...
  store: MongoStore.create({
    mongoUrl: process.env.MONGODB_URI || 'mongodb://localhost:27017/documentos-app',
...
mongoose.connect(process.env.MONGODB_URI || 'mongodb://localhost:27017/documentos-app')
...
app.post('/register', async (req, res) => {
  try {
    ...
    if (existingUser) {
      return res.render('register', { error: 'El usuario o email ya está registrado' });
    }
...
app.delete('/documents/:id/permanent', requireLogin, async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Error al eliminar el documento permanentemente' });
  }
});
// =============================================================================
// GESTIÓN DE PAPELERA
    ...
    const deletePromises = deletedDocuments.map(document => {
      return new Promise((resolve, reject) => {
        fs.unlink(document.filePath, (err) => {
          if (err) {
            console.error(`Error al eliminar archivo físico ${document.filePath}:`, err);
          }
          resolve();
        });
      });
...
app.put('/documents/:id/restore', requireLogin, async (req, res) => {
  try {
    ...
    if (!document) {
      return res.status(404).json({ error: 'Documento no encontrado en la papelera' });
    }
    ...
    res.status(500).json({ error: 'Error al restaurar el documento de la papelera' });
  }
});
...
app.put('/documents/:id/star', requireLogin, async (req, res) => {
  try {
    ...
    if (!document) {
      return res.status(404).json({ error: 'Documento no encontrado o está en la papelera' });
    }
    ...
    res.status(500).json({ error: 'Error al actualizar estado destacado del documento' });
  }
});
...
app.get('/api/profile', requireLogin, async (req, res) => {
  try {
    ...
    res.json({
      ...
      maskedEmail: user.email.substring(0, 3) + '*'.repeat(user.email.length - 3)
    });
  } catch (error) {
...
app.put('/api/profile/username', requireLogin, async (req, res) => {
  try {
    ...
    // Validar entrada
    if (!newUsername) {
      return res.status(400).json({ error: 'Debes proporcionar un nuevo nombre de usuario' });
    }
...
app.put('/api/profile/email', requireLogin, async (req, res) => {
  try {
    ...
    if (newEmail !== confirmEmail) {
      return res.status(400).json({ error: 'El nuevo correo y la confirmación no coinciden' });
    }
